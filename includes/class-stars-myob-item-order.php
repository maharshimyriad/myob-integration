...
/**
* Implements a class that represents a MYOB Order made up of physical Items
*/
class Opmc_Myob_Item_Order {
	...
	public function add_line_item( $item_id, $item_data, $tax_code ) {
		...
		// Determine Job and add validation
		$job_uid = null;
		$job_source = 'none';
		
		if ($product->get_meta('_myob_product_job_code', true)) {
			$job_uid = $product->get_meta('_myob_product_job_code', true);
			$job_source = 'product_meta';
		} else if (isset($wc_settings['WC_MYOB_job_code']) && !empty($wc_settings['WC_MYOB_job_code'])) {
			$job_uid = $wc_settings['WC_MYOB_job_code'];
			$job_source = 'global_setting';
		}

		// If Job UID is set, include it; otherwise set to null
		// Note: Job validation/existence check should be done on MYOB side
		$job = null;
		if ($job_uid) {
			$job = array('UID' => $job_uid);
			error_log('[MYOB Order] Line item Job UID set from ' . $job_source . ': ' . $job_uid . ' for product SKU: ' . $sku);
		}
	...
	public function add_line_professional( $item_id, $item_data, $tax_code ) {
		...
		if (isset($shipping) && 0 != $shipping) {
			...
			// Validate that income account is set before creating freight line
			if (empty($income_account)) {
				error_log('[MYOB Order] Warning: Income account is not set. Freight line cannot be created without a valid Account UID. Check System Settings > MYOB > Income Account.');
			} else {
				$line_item = array(
					'Type' => 'Transaction',
				...
				error_log('[MYOB Order] Freight line created with Account UID: ' . $income_account);
				/*End*/
				$this->line_items[] = $line_item;
			}
		}
	...
	public function generate_post_data() { 
		...
		$customer_po_number = 'WEB-' . $this->order_id;
		...
		// Always let MYOB generate the invoice number - never send custom Number field
		error_log('[MYOB Order] Invoice number will be auto-generated by MYOB. WooCommerce order ID: ' . $this->order_id . ' stored in CustomerPurchaseOrderNumber field.');
